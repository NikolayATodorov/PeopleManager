...
export class AddressComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAddress): number => this.addressService.getAddressIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IAddress[] | null): IAddress[] {
    ...
    if (this.links().prev) {
      const addressesNew = this.addresses ?? [];
      if (data) {
        for (const d of data) {
          if (addressesNew.map(op => op.id).indexOf(d.id) === -1) {
            addressesNew.push(d);
          }
        }
      }
